                 ...
                 unsigned int start_time, fluid_real_t gain)
{
  ...
  p = voice->mod;
  while (p != NULL) {
    fluid_mod_t *tmpmod = (fluid_mod_t *) p->data;
    FLUID_FREE(tmpmod);
    p->data = NULL;
    p = fluid_list_next(p);
  }
...
int fluid_voice_calc_amp(fluid_voice_t *voice) {
...
#ifdef FLUID_FIXED_POINT
  if ((voice->amp == 0) && (voice->amp_incr == 0))
    return 1;
#else
  ...
  voice->phase_incr = (uint64_t)(fluid_ct2hz_real(voice->pitch + voice->modlfo_val * voice->modlfo_to_pitch
                                 + voice->viblfo_val * voice->viblfo_to_pitch
                                 + voice->modenv_val * voice->modenv_to_pitch) / voice->root_pitch * 4294967296.0f);
...
int fluid_voice_iir_filter_calc(fluid_voice_t *voice) {
  ...
  {
    float amp = 1.0;
    float w = 2.0 * M_PI * fres / voice->output_rate; // Pole angle
    float q = 1.0 - w / (2.0 * (amp + 0.5 / (1.0 + w)) + w - 2.0); // Pole magnitude
    voice->r = q * q;
    voice->c = voice->r + 1.0 - 2.0 * FLUID_COS(w) * q;
  }
...
void fluid_voice_iir_filter_apply(fluid_voice_t *voice, int cnt) {
  float r = voice->r;
  float c = voice->c;
  fluid_buf_t *dsp_buf = voice->dsp_buf;

  float vibrapos = voice->vibrapos;
  float vibraspeed = voice->vibraspeed;
  ...
  voice->vibrapos = vibrapos;
  voice->vibraspeed = vibraspeed;
}
...
int fluid_voice_iir_filter_calc(fluid_voice_t *voice) {
  ...
  if ((FLUID_ABS(fres - voice->last_fres) > 0.01))
  {
    ...
    * bandwidth readjustment'. */
    ...
    fluid_real_t omega = (fluid_real_t) (2.0 * M_PI * (fres / ((fluid_real_t) voice->output_rate)));
...
void fluid_voice_iir_filter_apply(fluid_voice_t *voice, int count) {
  ...
  fluid_buf16_t dsp_a1_incr = voice->a1_incr;
  fluid_buf16_t dsp_a2_incr = voice->a2_incr;
  fluid_buf16_t dsp_b02_incr = voice->b02_incr;
  fluid_buf16_t dsp_b1_incr = voice->b1_incr;
  ...
  fluid_buf_t dsp_in, dsp_h1_a1, dsp_h2_a2, dsp_h1_b1, dsp_cn_h2, dsp_cn_b2;
...
//  if (FLUID_ABS(dsp_hist1) < 1e-20) dsp_hist1 = 0.0f;  /* FIXME JMG - Is this even needed? */
  ...
  */
  ...
  if (dsp_filter_coeff_incr_count > 0)
  {
    ...
    {
      dsp_in = dsp_buf[dsp_i];
      /* The filter is implemented in Direct-II form. */
      dsp_h1_a1 = FLUID_BUF_SAT32(FLUID_BUF_MULT32(dsp_a1, dsp_hist1));
      dsp_h2_a2 = FLUID_BUF_MULT32(dsp_a2, dsp_hist2);

      dsp_centernode = FLUID_BUF_SAT_SUB32(dsp_in, dsp_h1_a1);
      dsp_centernode = FLUID_BUF_SAT_SUB32(dsp_centernode, dsp_h2_a2);
      dsp_cn_h2 = FLUID_BUF_SAT_ADD32(dsp_centernode, dsp_hist2);
      dsp_cn_b2 = FLUID_BUF_MULT32(dsp_b02, dsp_cn_h2);
      dsp_h1_b1 = FLUID_BUF_MULT32(dsp_b1, dsp_hist1);
      dsp_buf[dsp_i] = FLUID_BUF_SAT_ADD32(dsp_cn_b2, dsp_h1_b1);
      dsp_hist2 = dsp_hist1;
...
// TODO
int fluid_voice_calc_effects(fluid_voice_t *voice, fluid_buf_t* dsp_reverb_buf, fluid_buf_t* dsp_chorus_buf, uint16_t cnt)
{
  fluid_buf_t *dsp_buf = voice->dsp_buf;
...
uint32_t fluid_voice_calc_stereo(fluid_voice_t *voice, fluid_buf_t* dsp_left_buf, fluid_buf_t* dsp_right_buf, uint32_t cnt)
{
  fluid_buf_t *dsp_buf = voice->dsp_buf;
...
uint32_t fluid_voice_interpolate(fluid_voice_t *voice)
{
  fluid_phase_t dsp_phase = voice->phase;
  fluid_phase_t dsp_phase_incr = voice->phase_incr;
  fluid_buf_t *dsp_buf = voice->dsp_buf;
  fluid_buf_t dsp_amp = voice->amp;
  fluid_buf_t dsp_amp_incr = voice->amp_incr;
  ...
  voice->amp = dsp_amp;
  return (FLUID_BUFSIZE - (dsp_cnt << 2));
}
...
int fluid_voice_write(fluid_voice_t* voice,
                  fluid_buf_t* dsp_left_buf, fluid_buf_t* dsp_right_buf,
                  fluid_buf_t* dsp_reverb_buf, fluid_buf_t* dsp_chorus_buf)
{
  ...
  if (voice->is_looping) {
    voice->end_index = voice->loopend;
    voice->loop_size = (uint64_t)(voice->loopend - voice->loopstart) << 32;
  }
  else {
    voice->end_index = voice->end;
    voice->loop_size = 0;
  }

  count = fluid_voice_interpolate(voice);
  ...
  if (voice->channel && fluid_channel_sustained(voice->channel)) {
    ...
    if (voice->volenv_section == FLUID_VOICE_ENVATTACK) {
      ...
      if (voice->volenv_val > 0) {
        ...
        fluid_real_t env_value = - ((-200 * FLUID_MATH_LOG(amp) / FLUID_MATH_LOG(10.0) - lfo) / 960.0 - 1);
